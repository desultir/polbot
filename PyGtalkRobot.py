...
class GtalkRobot:
    ...
    def setState(self, show, status_text):
	    ...
	    pres=xmpp.Presence(priority=5, show="available", status=self.status)
            self.conn.send(pres)
	    pres=xmpp.Presence(priority=5, show=None, status=self.status)
            self.conn.send(pres)
